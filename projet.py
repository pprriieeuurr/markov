...
class Noeud:
    """
    Classe permettant de stocker un noeud d'un graphe.

    Attributes
    ----------
    étiquette : str
        Le titre de ce noeud au format str.
    """
    def __init__(self,étiquette:str)->None:
        """
        Initialise une instance de Noeud.
        ...
        """
        assert type(étiquette) == str, f"Erreur de format, l'étiquette doit être de type str, et non pas de type {type(étiquette)}."
        self.étiquette = étiquette
    def __str__(self)->str:
        """
        Retourne l'étiquette de ce noeud.
...
class Graphe:
    ...
    def __init__(self)->None:
        ...
        """
        self.adj = {}
    def noeud_existe(self,éti_noeud:str)->bool:
        """
        Vérifie si un noeud est présent dans le graphe.

        Parameters
        ----------
        éti_noeud : str
            L'étiquette du noeud recherché.
    ...
    def noeuds(self)->list:
        """
        Permet de récupérer la liste des noeuds présents dans le graphe.

        Returns
        -------
        list :
            La liste des clés de self.adj et donc la liste des noeuds du graphe.
        """
        return list(self.adj.keys())
    def chercher_noeud(self,éti_noeud:str)->Noeud:
        """
        Retourne le noeud portant l'étiquette demandée, s'il n'éxiste pas on le crée dans le graphe.
        ...
        Noeud :
            Le noeud recherché.
        """
        assert type(éti_noeud) == str, f"Erreur de format, l'étiquette doit être de type str, et non pas de type {type(éti_noeud)}."
        for noeud in self.noeuds():
            if str(noeud) == éti_noeud:
                return noeud
        nouv_noeud = Noeud(éti_noeud)
        self.ajouter_noeud(nouv_noeud)
        return nouv_noeud
    def ajouter_noeud(self,nouv_noeud:Noeud)->None:
        """
        ...
        nouv_noeud : Noeud
            Le noeud à ajouter.
        """
        assert type(nouv_noeud) == Noeud, f"Erreur de format, le noeud doit être de type Noeud, et non pas de type {type(nouv_noeud)}."
        if nouv_noeud not in self.noeuds():
            self.adj[nouv_noeud] = []
    def ajouter_arête(self,noeud1:Noeud,noeud2:Noeud)->None:
        """
        ...
        """
        assert type(noeud1) == Noeud, f"Erreur de format, le premier noeud doit être de type Noeud, et non pas de type {type(noeud1)}."
        assert type(noeud2) == Noeud, f"Erreur de format, le deuxième noeud doit être de type Noeud, et non pas de type {type(noeud2)}."
        self.ajouter_noeud(noeud1)
        self.ajouter_noeud(noeud2)
        self.adj[noeud1].append(noeud2)
    def voisins(self,noeud:Noeud)->list:
        """
        ...
        """
        assert type(noeud) == Noeud, f"Erreur de format, le noeud doit être de type Noeud, et non pas de type {type(noeud)}."
    ...
    def visualiser(self)->None:
        ...
        for mot, derives in self.adj.items():
...
class GrapheMarkov(Graphe):
    ...
    def __init__(self,nom:str="étude")->None:
        ...
        self.trajets = []
        self.nom = nom
    def ajouter_trajet(self,liste_noeuds:list[Noeud])->None:
        """
        ...
        liste_noeuds : list[Noeud]
            Une liste d'objet de la classe Noeud qui peuvent se suivre lors de la méthode héritage. Attention, tous les trajets doivent être de la même taille.
        """
    ...
    def héritage(self,liste_noeuds:list[Noeud])->Noeud:
        """
        ...
        """
        assert type(liste_noeuds) == list and 0<len(liste_noeuds) and len(liste_noeuds)<=len(self.trajets[0]) and all(type(noeud) == Noeud for noeud in liste_noeuds), "La liste fournie ne répond pas aux critères."
        trajets_possibles = []
        ...
        return choice(self.noeuds()) # Si aucun élément n'as été trouvé, on retourne un noeud aléatoire.
    def __str_from_liste(self,mots_de_début:list[str],taille:int=15):
        """
        ...
        """
        liste_parents = [self.chercher_noeud(mot) for mot in mots_de_début]
        retournement = ""
        for parent in liste_parents:
            retournement += str(parent) + " "
        for i in range(taille-len(mots_de_début)):
            liste_parents.append(self.héritage(liste_parents))
            liste_parents.pop(0)
            retournement += str(liste_parents[-1]) + " "
        return retournement
    def ajouter_paragraphe(self,p:str,taille_trajets:int=6)->None:
        ...
        for i in range(1,taille_trajets): # Boucle permetant d'ajouter les premeiers mots dans le graphe et leur trajets pour les cas où on utilise un niveau d'intelligence moins fort que le maximum.
            self.ajouter_arête(self.chercher_noeud(liste_mots[i-1]),self.chercher_noeud(liste_mots[i]))
            ...
            liste_trajet.extend([self.chercher_noeud(liste_mots[i+j]) for j in range(-i+1,1)])
        ...
        for i in tqdm(range(taille_trajets,len(liste_mots)-1)): # On répète pour tous les mots non-créés.
            self.ajouter_arête(self.chercher_noeud(liste_mots[i-1]),self.chercher_noeud(liste_mots[i]))
            self.ajouter_trajet([self.chercher_noeud(liste_mots[i+j]) for j in range(-taille_trajets,0)]) # Permet de gérer la longueur n grâce aux indice.
        self.ajouter_arête(self.chercher_noeud(liste_mots[-2]),self.chercher_noeud(liste_mots[-1]))
        self.ajouter_trajet([self.chercher_noeud(liste_mots[len(liste_mots)+j]) for j in range(-taille_trajets,0)]) # Ajoute le dernier trajet.
    def str(self,taille:int=100,mot_de_début:str=None,intel:int=3):
        ...
        if mot_de_début is not None and self.noeud_existe(mot_de_début): # Met le premier élément dans liste_parents.
            liste_parents = [self.chercher_noeud(mot_de_début)] 
        else:
            liste_parents = [choice(self.noeuds())]
